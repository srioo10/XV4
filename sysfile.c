...
int sys_version_list(void) {
  ...
  if (argstr(0, &path) < 0 || argptr(1, (void *)&buf, 0) < 0 ||
  ...
  while (vblock != 0) {
    ...
    if (current_version == version_num) {
      ...
      for (int i = 0; i < vnode->nblocks && i < NDIRECT; i++) {
        ...
        if (newblock == 0)
          break; // Should handle error better but panic avoidance
...
int sys_version_get_content(void) {
  char *path;
  int version_num;
  char *buf;
  int bufsize;
  ...
  if (argstr(0, &path) < 0 || argint(1, &version_num) < 0 ||
      argint(3, &bufsize) < 0 || argptr(2, &buf,bufsize) < 0)
    return -1;
  ...
  while (vblock != 0) {
    ...
    if (current_version == version_num) {
      // Found the target version - read its content
      int total_read = 0;
      int to_read = vnode->file_size < bufsize ? vnode->file_size : bufsize;

      // Read data from version blocks
      for (int i = 0; i < vnode->nblocks && total_read < to_read; i++) {
        struct buf *bp = bread(ip->dev, vnode->data_blocks[i]);
        int bytes_to_copy = to_read - total_read;
        if (bytes_to_copy > BSIZE)
          bytes_to_copy = BSIZE;

        // Copy to user buffer using copyout
        if (copyout(curproc->pgdir, (uint)buf + total_read, bp->data,
                    bytes_to_copy) < 0) {
          brelse(bp);
          iunlockput(ip);
          end_op();
